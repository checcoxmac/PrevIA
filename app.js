...
function renderJobs() {
  ...
  wrap.innerHTML = filtered.map(job => {
    ...
    return `
      <article class="list-row" data-job="${job.id}" style="position:relative;">
        <div class="left">
        ...
        <button type="button" class="btn-ghost danger btn-compact" data-job-del="${job.id}" style="position:absolute;top:8px;right:8px;"><i class="fa-solid fa-trash"></i></button>
      </article>`;
  }).join("");
...
function printQuote(id) {
  ...
  const rowsHTML = quote.righe.map(r => {
    ...
    return `<tr>
      <td style="border:1px solid #ddd;padding:8px;">${escapeHTML(r.desc)}</td>
      <td style="border:1px solid #ddd;padding:8px;text-align:center;">${r.qty}</td>
      <td style="border:1px solid #ddd;padding:8px;text-align:right;">${formatMoney.format(r.unitPrice)}</td>
      <td style="border:1px solid #ddd;padding:8px;text-align:center;">${r.sconto}%</td>
      <td style="border:1px solid #ddd;padding:8px;text-align:center;">${r.iva}%</td>
      <td style="border:1px solid #ddd;padding:8px;text-align:right;font-weight:700;">${formatMoney.format(tot)}</td>
    </tr>`;
  }).join("");
  
  root.innerHTML = `
    <div style="font-family:'Inter',sans-serif;padding:40px 32px;color:#0f172a;max-width:800px;margin:0 auto;">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:20px;border-bottom:2px solid #4f46e5;padding-bottom:16px;margin-bottom:24px;">
        <div style="flex:1;">
          ${logoHTML}
          <h1 style="margin:4px 0 8px 0;font-size:20px;font-weight:700;">${escapeHTML(state.companyName)}</h1>
        ...
        <div style="text-align:right;min-width:180px;">
          <p style="margin:0 0 8px 0;font-size:28px;font-weight:800;color:#4f46e5;letter-spacing:-0.02em;">PREVENTIVO</p>
      ...
      <div style="margin:0 0 24px 0;padding:16px;background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0;">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:13px;margin-bottom:24px;">
        <thead>
          <tr style="background:#f8fafc;">
            <th style="border:1px solid #e2e8f0;padding:10px 8px;text-align:left;font-weight:700;">Descrizione</th>
            <th style="border:1px solid #e2e8f0;padding:10px 8px;text-align:center;font-weight:700;width:60px;">Qta</th>
            <th style="border:1px solid #e2e8f0;padding:10px 8px;text-align:right;font-weight:700;width:100px;">Prezzo</th>
            <th style="border:1px solid #e2e8f0;padding:10px 8px;text-align:center;font-weight:700;width:70px;">Sconto</th>
            <th style="border:1px solid #e2e8f0;padding:10px 8px;text-align:center;font-weight:700;width:60px;">IVA</th>
            <th style="border:1px solid #e2e8f0;padding:10px 8px;text-align:right;font-weight:700;width:100px;">Totale</th>
          </tr>
        </thead>
      ...
      ${quote.notes ? `<div style="margin-top:24px;padding:14px;border-radius:8px;background:#fef3c7;border:1px solid #fde68a;color:#92400e;font-size:13px;line-height:1.6;"><strong>Note:</strong><br/>${escapeHTML(quote.notes)}</div>` : ""}
  ...
  // Force reflow to ensure layout is ready before printing
